...
staff.dat = staff.dat %>% select(infection_day_incub5_outcome, date, pt_infectpa_binary_incub5_comm, pt_infectpa_binary_incub5_noso, comm_infectpa,
...
load('analysis_code/pt.gam.mod.onlycal_190221.Rdata')
...
load('analysis_code/pt.gam.mod_190221.Rdata')
...
cal.plot = ggplot(cal.plot.dat, aes(x = x, y = y, group = var, color = var, linetype = var)) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = var),  color = NA, alpha = 0.2) + 
  geom_line() +
...
foi.plot = ggplot(foi.plot.dat, aes(x = x, y = y, group = var, color = var)) + 
  ...
  scale_color_manual(name = 'Types of infectious individual', 
                       values = c('#0C0A3E','#EF767A',"#EEB868"),
                       labels = c('Patients with likely community-acquired COVID-19', 
                                  'Patients with potentially hospital-acquired COVID-19',
                                  'Health care staff')) + 
  scale_fill_manual(values = c('#0C0A3E','#EF767A',"#EEB868"), guide = 'none') +
...
pgam = ggarrange(cal.plot, foi.plot, ncol = 2)
...
load('analysis_code/staff.gam.mod.onlycal_190221.Rdata')
...
#                      family=binomial(link = 'logit'),
#                      data = staff.dat)
# save(staff.gam.mod, file = 'analysis_code/staff.gam.mod_150721.Rdata')

load('analysis_code/staff.gam.mod_150721.Rdata')
...
pt.comm.admit = get.plot.dat(mod = staff.gam.mod, var.no = 5) 

cal.plot.dat = rbind.data.frame(cal.only, cal)
cal.plot.dat$lower = cal.plot.dat$y - cal.plot.dat$se
cal.plot.dat$upper = cal.plot.dat$y + cal.plot.dat$se

foi.plot.dat = rbind.data.frame(pt.comm, pt.noso, staff, pt.comm.admit)
foi.plot.dat$lower = foi.plot.dat$y - foi.plot.dat$se
foi.plot.dat$upper = foi.plot.dat$y + foi.plot.dat$se
...
cal.plot = ggplot(cal.plot.dat, aes(x = x, y = y, group = var, color = var, linetype = var)) + 
  ...
  scale_x_continuous(breaks= c(1, lubridate::yday(as.Date('2020-02-01')),
                     ...
                     labels = c('January','February', 'March', 
                                ...
                                'October'),
                     limits = c(61, 250))+
  scale_y_continuous(limits = c(-3.2, 20), breaks = seq(0, 20, by = 5))+
  scale_linetype_manual(values = c('solid', 'dashed'), guide = 'none')+
  scale_color_manual(values = c('grey20', 'grey80'),
                     name = '', 
...
foi.plot = ggplot(foi.plot.dat, aes(x = x, y = y, group = var, color = var)) + 
  ...
  scale_x_continuous(limits = c(0, 4)) +
  scale_color_manual(name = 'Types of infectious individual', 
                       values = c('#0C0A3E','#EF767A',"#EEB868",'#87C38F'),
                       labels = c('Patients with likely community-acquired COVID-19', 
                                  'Patients with potentially hospital-acquired COVID-19',
                                  'Health care staff', 
                                  'Community COVID-19')) + 
  scale_fill_manual(values = c('#0C0A3E','#EF767A',"#EEB868",'#87C38F'), guide = 'none') +
  ylab('Partial effect of infectious individuals on daily\nrisk of nosocomial SARS-CoV-2 infection') + 
  xlab('') + 
  theme_minimal() + 
  theme(text = element_text(size = 15), 
...
sgam = ggarrange(cal.plot, foi.plot, ncol = 2)

ggarrange(pgam, sgam, nrow = 2, hjust= -0.2,
          labels = c('A. Daily SARS-CoV-2 transmission risk to susceptible patients',
                     'B. Daily SARS-CoV-2 transmission risk to susceptible healthcare workers'))

ggsave('manuscript/gam_all.png', dpi = 500, unit = 'in', width = 13, height = 11)
...
load('analysis_code/pred_mod_pt190221.Rdata')
...
# we are presumably not considering patients who test positive the in the first 48hrs after admission here
p = ggplot(data = df, aes(x = day.of.stay, y = probs, group = month, color = month)) +
  ...
  ylab('Daily probability of having a first positive\nSARS-CoV-2 PCR test while hospitalised') + 
